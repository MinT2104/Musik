...
const NavMob = ({dataNav, setDataNav}) => {
    ...
    const li_nav = " w-full text-center m-4 pl-12 font-semibold hover:border-r-4 hover:border-[#FFC23C] hover:text-[#FFC23C] duration-300 flex flex-row justify-left gap-2 items-center"
   const navPages = [
    {
        id:1,
        title: "Home",
            ...
            <nav> 
                ...
                {navPages.map((item, index) =>(
                     ...
                     onClick={()=>{
                        ...
                        navigate(localStorage.getItem("activepath"))
                        }}
                     className={`${activePath === item.path && activeLink || "text-gray-400"} ${li_nav}`}>
                     
                     {Parser().parse(item.logo)}
                    <Link to={activePath} className="hover:translate-x-1.5 duration-300">{item.title}</Link>
                   </li> 
